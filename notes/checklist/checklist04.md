...
커밋 로그는 작업도중 어떠한 작업이 완성되면 해당 작업이력을 의미한다. 팀원들은 커밋 로그를 통해 프로젝트가 어떻게 / 어디가 수정되었는지 인지 할수 있으며 이후의 작업을 어떻게 진행 하는지에 대한 척도로 사용할 수 있다.. 그러므로 커밋 로그는 팀원 모두 함께 룰을 정하는 것이 좋으며, 간결하고 알기 쉽게 작성해야한다.  가독성이 좋고 명시적이며, 규칙적인 커밋로그가 좋다. 
...
커밋은 할수 있으면 자주, 작은 단위로 하는것이 좋다고 생각한다. 커밋은 "마무리된 작업의 작업이력을 남기는 작업" 으로 볼수 있지만, 이후에 진행 된 부분이 잘못 된 방향으로 가고 있다면 다시 되돌리기위한 수단이라고 볼수 있다. 그렇기 때문에 작은 단위로 커밋을 하게 되면 프로젝트의 방향이 잘 못 되어 다시 돌아가도 많은 피해 없이 새로운 작업을 할수 있는 장점이 있다. 그렇다고 너무 작은 단위로는 하지 않고 함수의 기능 완성히 커밋을 하는 것이 좋다고 생각한다.
...
master branch를 서비스 출시 하기 전까지 절대 merge 하면 안된다.
...
function 과 arrow function 의 차이는 bind에 차이가 존재 한다. javascript 에서 this는 실행 하는 타이밍에 바인딩 되기 때문에 일반 function은 .bind() 함수를 통해 context를 지정 할수 있다. 하지만 arrow function은 자동 바인딩이 되기 때문에 callback function을 사용할 때 많이 사용한다.
